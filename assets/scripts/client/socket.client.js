...
function connectToServer() {
	...
	connection.onmessage = function (message) {
		...
		if(json.type == "notification") {
			...
			addTransaction(name, json.data.reason, json.data.amount, json.data.timestamp, isPositive);
			...
			addTransaction(name, t.reason, t.amount, t.timestamp, isPositive);
			});
		}
	}
}
...
const performLogin = (name) => {
	...
	sendObject(connection, "login", data);
};

const performSendMoney = (receiver, amount) => {
	let data = {
		receiver: receiver,
		amount: amount
	}
	sendObject(connection, "sendMoney", data);
}
